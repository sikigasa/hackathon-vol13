...
Future<Uint8List> saveJpeg(Uint8List jpegBytes) async {
  try {
    // Step 2: 保存先のディレクトリを取得

    final Directory directory = await getApplicationDocumentsDirectory();
    final picturesPath = await ExternalPath.getExternalStoragePublicDirectory(
      ExternalPath.DIRECTORY_PICTURES,
    );
    final albumPath = '$picturesPath/ocr_test';
    var dir = await Directory(albumPath).create(recursive: true);

    final path = '${dir.path}/ocr_test.jpg';
    final file = File(path)..writeAsBytesSync(jpegBytes);
    final String filePath = directory.path;

    await PhotoManager.editor.saveImageWithPath(
      file.path,
      title: "ocr_test.jpg",
      relativePath: "ocr_test",
    );

    print('画像を保存しました: $filePath');
  } catch (e) {
    print('エラー: $e');
  }
  return jpegBytes;
